/**
 * TodaydeskAudioController — pickup / drop / done SFX.
 * Expects nothing from the scene besides this host object. Does not own
 * board state or UI.
 */

const PICKUP_TRACK = requireAsset("../GeneratedSFX/CardPickup.wav") as AudioTrackAsset
const DROP_TRACK = requireAsset("../GeneratedSFX/CardDrop.wav") as AudioTrackAsset
const DONE_TRACK = requireAsset("../GeneratedSFX/CardDone.wav") as AudioTrackAsset

@component
export class TodaydeskAudioController extends BaseScriptComponent {
  @ui.label('<span style="color: #60A5FA;">TodaydeskAudio – quiet grab/drop cues</span>')
  @ui.separator
  @ui.group_start("Settings")
  @input
  @hint("Pickup volume (0–1)")
  @widget(new SliderWidget(0, 1, 0.05))
  pickupVolume: number = 0.45
  @input
  @hint("Drop volume (0–1)")
  @widget(new SliderWidget(0, 1, 0.05))
  dropVolume: number = 0.5
  @input
  @hint("Done-tray settle volume (0–1)")
  @widget(new SliderWidget(0, 1, 0.05))
  doneVolume: number = 0.55
  @ui.group_end
